...
async function handleAddGroup(chatId, text) {
  ...
  return sendMessage(
    ...
    `✅ Группа «${escapeHtml(name)}» создана и выбрана для этого чата.\n\n` +
      ...
      `(или через сайт, если там есть форма добавления) — и /today заработает.`
  );
}
...
async function handleToday(chatId) {
  ...
  if (!groupId) return sendMessage(chatId, "Сначала выбери группу: /groups (или создай новую: /addgroup Название)");

  const today = jsDayToOur(new Date().getDay());
  if (today === 7) return sendMessage(chatId, "Сегодня воскресенье, пар нет 🎉");

  const lessons = await getScheduleForDay(groupId, today);
  if (lessons.length === 0) return sendMessage(chatId, `Сегодня (${DAY_NAMES_FULL[today]}) пар нет.`);
...
async function handleNow(chatId) {
  ...
  if (lessons.length === 0) return sendMessage(chatId, "На сегодня пар нет.");
  ...
  if (next) {
    return sendMessage(chatId, `Сейчас перемена. Следующая пара в ${hhmm(next.time_start)}: ${escapeHtml(next.subject_name)}`);
  }

  return sendMessage(chatId, "На сегодня все пары закончились");
}
  ...
  try {
    ...
    if (update.message) {
      ...
      if (text.startsWith("/start")) {
        ...
        await sendMessage(chatId, "Не понимаю команду. Напиши /start, чтобы увидеть список команд.");
      }
    }
